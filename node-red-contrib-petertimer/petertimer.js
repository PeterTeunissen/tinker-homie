...
module.exports = function(RED) {
				...
				.on(
						...
						function(inmsg) {
						   ...
						   // now=new Date(now+nowOff); // from now on we're working on local time		
							...
							var outmsg3 = {
								payload : { "start" : pad(parseInt(actualStartTime /60),2) + ":" + pad(actualStartTime %60,2),
									     "end" : pad(parseInt(actualEndTime/60),2) + ":" + pad(actualEndTime%60,2)
								},
								topic : "",
							};
							...
							if ((change) || ((node.atStart)&&(startDone==0)))
							{
								if (outmsg.payload>"") 
								{
									if (stopped==0) node.send([outmsg, outmsg2,outtext,outmsg3]);	
								}									
								 else 
								 {
									 if (stopped==0) node.send([null, outmsg2,outtext,outmsg3]);
								 }
							}
							else
							{
								...
								{
									if (node.repeat) 
									{ if (stopped==0) node.send([outmsg, outmsg2,null,outmsg3]);}
								else 
									{ if (stopped==0) node.send([null, outmsg2,null,outmsg3]); }
	...
	RED.nodes.registerType("petertimer", petertimerNode);
}
